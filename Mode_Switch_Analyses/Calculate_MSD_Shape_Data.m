function [MSD, MSD_error, MSD_AMME, MSD_AMME_error, MSD_All, MSD_All_error]=Calculate_MSD_Shape_Data(locs, frames, classes)
    %{
    Author: Chris Eddy
    Date: 05/03/22
    ...
    Likely, a more general script that Calculate_MSD_Data.m

    Input Arguments:

        locs:   cell array, 1 x NumCells. Contains the x positions of cell
                 i in locs{i}
    ...
    for i=1:length(locs)
        ...
        x = locs{i}; %trajectory ith x position
        for t = 1:length(frs)-1
            ...
            for tau_i = 1:length(taus)
                ...
                if length(Squared_Displacements{cs_cont(1)})<taus(tau_i)
                    Squared_Displacements{cs_cont(1)}{taus(tau_i)} = sum(xdiff.^2);
                else
                    Squared_Displacements{cs_cont(1)}{taus(tau_i)}(end+1)= sum(xdiff.^2);
                end
            end
        end
    end
        ...
        for t = 1:length(frs)-1
            ...
            for tau_i = 1:length(taus)
                ...
                if length(Squared_AMME_Displacements{cs_cont(1)})<taus(tau_i)
                    Squared_AMME_Displacements{cs_cont(1)}{taus(tau_i)} = sum(xdiff.^2);
                else
                    Squared_AMME_Displacements{cs_cont(1)}{taus(tau_i)}(end+1) = sum(xdiff.^2);
                end
            end
        end
    end
    ...
    for i=1:length(locs)
        ...
        for t=1:length(frs)-1
            ...
            taus = cumsum(g);
            for tau_i = 1:length(taus)
                xdiff = locs{i}(t+tau_i,:) - locs{i}(t,:);
                Sq_diff_all{taus(tau_i)}(end+1) = sum(xdiff.^2);
            end
        end
    end
    MSD_All = zeros(1, L);
